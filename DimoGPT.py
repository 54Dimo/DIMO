...
def excerciseAndmeal(user_msg,user_id):
    ...
    if len(mealList)==0 or mealList=='noTable':
        metaPrompt=f''' 請根據下面提供的個人資訊，給予此問題最多三個建議
...
def eating_method_sugest(user_msg,user_id):
    ...
    food_had_eat=[]
    if len(mealList)==0 or mealList=='noTable':
        metaPrompt=f''' 請根據針對以下個人資訊，給予1~3個重點來回覆此問題:
        「我是一位{user_info.get('gender')}、身高:{user_info.get('height')}公分、體重:{user_info.get('weight')}公斤
        平時從事{user_info.get('exerciseIntensity')}運動，想要達成{user_info.get('fitnessGoal')}的體態目標。
        今天還沒有吃過任何東西。」
        '''
...
def resipeOffering(dish_name,user_id):
    metaPrompt=f'''請根據現有資料，或是利用google_res function，提供{dish_name}的相關食譜或式料理方式。
...
func_table = [
    ...
    {                       # 每個元素代表一個函式
        ...
        "spec": {           # function calling 需要的函式規格
            "name": "food_info",
            "description": "回覆「食品營養資訊」此類型問題，可以詢問食品的營養資訊。",
            "parameters": {
                "type": "object",
                "properties": {
                    "food_name": {
    ...
    {                       # 每個元素代表一個函式
        ...
        "spec": {           # function calling 需要的函式規格
            "name": "resipeOffering",
            "description": "回覆「食譜推薦或料理方式」此類型問題，提供菜品料理方式或詳細食譜，若使用者語意內包含食物或式菜餚名稱，即可使用此function",
            "parameters": {
                "type": "object",
                "properties": {
                    "dish_name": {
...
def UsingChat(user_id,hist,user_msg):  
    ...
    sys_msg='請根據main question內容判斷出對應的function進行回覆，若無法選擇對應的function，請回覆:「本問題無法回應，請詢問別的問題。」'
